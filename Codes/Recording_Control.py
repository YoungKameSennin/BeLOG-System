...
try:
    while True:
        ...
        if recording:
            if colorwriter is None or depthwriter is None:
                ...
                color_path = os.path.join(
                    recordings_folder, f'{current_time}_rgb.avi')
                depth_path = os.path.join(
                    recordings_folder, f'{current_time}_depth.avi')
                audio_path = os.path.join(
                    recordings_folder, f'{current_time}_audio.wav')
        ...
        if not recording:
            ...
            events_list = []
            log_time = time.strftime("%T-%m/%d/%Y") + ": "
            print(log_time + "Recording started. Subject" +
        ...
        if keyboard.is_pressed('ctrl+alt+r'):
            ...
            if start_time:
                ...
                log_time = time.strftime("%T-%m/%d/%Y") + ": "
                print(log_time + "Recording stopped. Duration:",
                      time_converter(duration))
                print(log_time + "Saving the observation...")
                with open(logger_path, 'a') as file:
                    print(log_time + "Recording stopped. Duration:",
                          time_converter(duration), file=file)
                ...
                if audio_frames:
                    with wave.open(audio_path, 'wb') as audio_file:
                        ...
                        audio_file.setsampwidth(
                            p.get_sample_size(audio_format))
                ...
                if index != -1:
                    ...
                    with open(project_file_name, 'w') as file:
                        ...
                        with open(logger_path, 'a') as file:
                            print(
                                log_time + "Observation saved successfully.\n", file=file)
